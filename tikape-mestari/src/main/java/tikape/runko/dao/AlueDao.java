...
public class AlueDao implements Dao<Alue, Integer> {
    ...
    public String etsiAlueenOtsikko(Integer key) throws SQLException { //Listaa tietyn alueen kaikki viestiketjut
        ...
        ("SELECT Alue.a_id, Alue.otsikko, Alue.luoja, Alue.kuvaus, Alue.luomisaika, " +
                "COUNT(Viesti.v_id) AS v_maara, COUNT(DISTINCT Viestiketju.vk_id) AS vAloitusMaara, MAX(Viesti.lahetysaika)AS viimeinenV " + 
        ...
        while (rs.next()) {
            ...
            int va_maara = rs.getInt("vAloitusMaara");
     
            alueet.add(new Alue(a_id, otsikko, luoja, kuvaus, luomisaika, ViimeinenViestiAika, v_maara + va_maara));
        }
